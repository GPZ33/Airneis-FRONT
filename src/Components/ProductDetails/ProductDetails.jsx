...
const ProductDetails = () => {
        ...
        if (product && product.materials && product.materials.length > 0) {
          const materialsData = await Promise.all(
            product.materials.map(
              async (material) =>
                await productApiService.getMaterialsOfProductById(
                  material["@id"]
                )
            )
          );
  ...
  useEffect(() => {
    if (product && product.category && product.category.length > 0) {
      categoryApiService
        .getCategoryOfProduct(product.category[0]["@id"])
        .then((result) => {
        ...
        <>
          <div>
            <article>
              <div className="photo">
                <CarouselSlides
                  props={images.map((image) => image.contentUrl)}
                  largerHeight={true}
                />
              </div>
              <div className="description text-white">
                ...
                <div>
                  <p className="pt-3 text-white">Détails: {product.details}</p>
                  <p className="pt-3 text-white">
                    Category:{" "}
                    {categoryOfProduct ? categoryOfProduct.name : "Loading..."}
                  </p>
          ...
          <div className="row justify-content-around">
            ...
            {categoryOfProduct && categoryOfProduct.id && (
              <SimilarProducts idCategoryOfProduct={categoryOfProduct.id} />
            )}
          </div>
        </>
      ) : (
