...
import { Button, TextField, Checkbox, FormControlLabel, CircularProgress, MenuItem, InputLabel, FormControl } from '@mui/material';
...
const ProductCreation = () => {
  ...
  const handleMaterialSearch = (event) => {
    ...
    const filtered = searchText === '' ? materials : materials.filter(material => material.name.toLowerCase().includes(searchText));
    setFilteredMaterials(filtered);
  };

  const handleImageSearch = (event) => {
    const searchText = event.target.value.toLowerCase();
    const filtered = images.filter(image => image.contentUrl.toLowerCase().includes(searchText));
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <FormControl fullWidth margin="normal">
        ...
        >
          <TextField label="Search" onChange={handleMaterialSearch} fullWidth margin="normal" />
      ...
      <FormControl fullWidth margin="normal">
        ...
        >
          <TextField label="Search" onChange={handleImageSearch} fullWidth margin="normal" />
      ...
      <FormControlLabel control={<Checkbox checked={stock} onChange={(e) => setStock(e.target.checked)} />} label="Stock" />
      <FormControlLabel control={<Checkbox checked={highlander} onChange={(e) => setHighlander(e.target.checked)} />} label="Highlander" />
